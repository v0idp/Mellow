...
class Database {
    ...
    saveSettings(request) {
        if (request.path == '/general') {
            ...
            this.db.run('INSERT INTO ombi (host, port, apikey, requesttv, requestmovie, approvetv, approvemovie, username) VALUES(?, ?, ?, ?, ?, ?, ?, ?)',
                [request.body.host, request.body.port, request.body.apiKey, request.body.requestTV, request.body.requestMovie, request.body.approveTV, request.body.approveMovie, request.body.userName]);
    ...
    init() {
        return new Promise(async (resolve, reject) => {
            this.openDB(this.path).then(db => {
                ...
                this.db.run('CREATE TABLE IF NOT EXISTS ombi (id integer primary key asc, host text, port text, apikey text, requesttv text, requestmovie text, approvetv, approvemovie, username text)');
